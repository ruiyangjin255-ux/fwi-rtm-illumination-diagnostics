...
outDir = 'D:\ryjin\paper_figures_source\vti\layer_thomsen_near_receivers_by_time';
...
stageNames = { ...
    ...
    'later_reflection_transmission_conversion' ...
};
...
for k = 1:numel(S.plot_times)
    ...
    outName = sprintf('vti_layer_thomsen_%s_t%03dms_vx_vz.png', ...
        stageNames{k}, round(S.plot_times(k) * 1000));
    exportgraphics(fig, fullfile(outDir, outName), 'Resolution', 300);
    close(fig);
end

fprintf('Single-time wavefield figures exported to: %s\n', outDir);
