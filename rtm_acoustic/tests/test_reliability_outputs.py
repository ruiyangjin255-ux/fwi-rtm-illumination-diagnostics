...
CONFIG = Path("rtm_acoustic/configs/salt_reliability_gate_v1.yaml")
...
def test_replay_and_ablation_generate_real_gate_outputs(tmp_path: Path) -> None:
    output = Path("rtm_acoustic/outputs/reliability_script_smoke")
    ...
    config.write_text(
        "\n".join(
            [
                ...
                "output_dir: rtm_acoustic/outputs/reliability_script_smoke",
                "num_shot_groups: 2",
    ...
    replay = subprocess.run(
        [sys.executable, "rtm_acoustic/scripts/replay_fwi_diagnostics.py", "--config", str(config), "--smoke"],
        cwd=Path.cwd(),
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    ...
    ablation = subprocess.run(
        [sys.executable, "rtm_acoustic/scripts/run_reliability_gate_ablation.py", "--config", str(config), "--smoke"],
        cwd=Path.cwd(),
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
