...
$LogDir = Join-Path $Root "rtm_acoustic\outputs\FWI\deep_time_multiscale_logs"
New-Item -ItemType Directory -Force -Path $LogDir | Out-Null

$Model = "rtm_acoustic\outputs\generated_inputs\seg676x230_from_fwi_true.bin"
$Common = @(
  "--model", $Model,
  "--nt", "5000",
  "--iterations", "2",
...
function Run-Stage {
  ...
  $args = @("rtm_acoustic\run_full_salt_fwi.py") + $Common + @("--f0", $F0, "--output-dir", $OutputDir)
  if ($InitialModel -ne "") {
...
$StageA = "rtm_acoustic\outputs\FWI\full_salt_fwi_deep_time_f4_audit0_train_pmlpad_v1"
$StageB = "rtm_acoustic\outputs\FWI\full_salt_fwi_deep_time_f6_audit0_train_pmlpad_v1"
$StageC = "rtm_acoustic\outputs\FWI\full_salt_fwi_deep_time_f8_audit0_train_pmlpad_v1"
