...
| 图号 | 内容 | 已有/建议路径 |
|---|---|---|
| 图 1 | FWI 模型质量门控与更新尺度选择 | `D:\ryjin\rtm_acoustic\docs\jge_main_figures\figure1_fwi_quality_gate.tiff` |
...
本文使用的代码、参数文件和主要输出位于本地项目 `D:\ryjin\rtm_acoustic`。推荐复现入口为 `python -m rtm_acoustic.run_optimized_fwi_rtm_pipeline`；若需要重新执行耗时 RTM 验证，可添加 `--run-rtm`。投稿材料打包入口为 `python -m rtm_acoustic.build_jge_submission_package`，输出目录为 `D:\ryjin\rtm_acoustic\docs\jge_submission_package`。完整 FWI 结果位于 `D:\ryjin\rtm_acoustic\outputs\FWI\full_salt_fwi_cg_allshots_v2`；优化 pipeline 总报告位于 `D:\ryjin\rtm_acoustic\outputs\FWI\full_salt_fwi_cg_allshots_v2\optimized_fwi_rtm_pipeline`；RTM before/after FWI 结果位于 `D:\ryjin\rtm_acoustic\outputs\RTM\before_after_fwi_alpha010_nt1200_shots12`；RTM 方案二结果位于 `D:\ryjin\rtm_acoustic\outputs\RTM\seg_salt_scheme2_full30m_nt4001_workers4`；局部 FWI 对照结果位于 `D:\ryjin\rtm_acoustic\outputs\FWI影响因素`。若投稿，需将可公开代码、参数配置、压缩后的结果表和可复现实验脚本整理到 GitHub/Zenodo，并避免上传受版权限制的原始模型文件。
