...
- 已有基础 FWI 脚本：`D:\ryjin\rtm_acoustic\run_small_salt_fwi.py`。
- 已有基础输出目录：`D:\ryjin\rtm_acoustic\outputs\small_salt_fwi_demo`。
- 已有中文说明：`D:\ryjin\rtm_acoustic\docs\small_salt_fwi_and_illumination.md`。
...
`D:\ryjin\rtm_acoustic\outputs\small_salt_fwi_illumination_compare`
...
更新 `D:\ryjin\rtm_acoustic\docs\small_salt_fwi_and_illumination.md`，补充以下差异：
