...
from admit_fwi.build_jge_method_synthesis import build as build_method_synthesis
...
from admit_fwi.check_jge_submission_readiness import check_manuscript, write_report
...
def _copy_pipeline_report(src: Path, dst: Path) -> dict[str, Any]:
    ...
    lines = [
        ...
        "Re-run the full local pipeline with `python -m admit_fwi.run_optimized_fwi_rtm_pipeline --run-rtm` when the raw SEG/Salt-derived data products are available.",
    ]
    dst.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return {
