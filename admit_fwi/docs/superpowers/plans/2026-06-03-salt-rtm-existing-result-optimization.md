...
**文件：**
- 新增：`D:\ryjin\admit_fwi\tests\test_optimize_existing_salt_result.py`
- 新增：`D:\ryjin\admit_fwi\optimize_existing_salt_result.py`
...
命令：`python -m pytest admit_fwi\tests\test_optimize_existing_salt_result.py -q`

预期：在实现前失败，原因是 `admit_fwi.optimize_existing_salt_result` 或新函数不存在。
...
**文件：**
- 修改：`D:\ryjin\admit_fwi\optimize_existing_salt_result.py`
...
命令：`python -m admit_fwi.optimize_existing_salt_result --input-dir admit_fwi\outputs\seg_salt_multishot_rtm_padded60_full30m_workers4`
