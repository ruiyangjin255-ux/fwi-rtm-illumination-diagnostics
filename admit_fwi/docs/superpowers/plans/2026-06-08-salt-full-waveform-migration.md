...
**Architecture:** 新增独立脚本复用 `admit_fwi.acoustic_rtm` 的多炮 RTM 接口，不改动已有 RTM/FWI 主流程。脚本负责模型裁剪、迁移速度构造、两类 RTM 运行、指标统计、图件保存和中文 Markdown 总结。

**Tech Stack:** Python、NumPy、Matplotlib、pytest、现有 `admit_fwi.acoustic_rtm` 声波 RTM 工具。
...
**Files:**
- Create: `D:\ryjin\admit_fwi\run_salt_full_waveform_migration.py`
- Test: `D:\ryjin\admit_fwi\tests\test_salt_full_waveform_migration.py`
...
Run: `python -m pytest D:\ryjin\admit_fwi\tests\test_salt_full_waveform_migration.py::test_default_crop_and_shots_are_inside_model -q`
...
from admit_fwi.acoustic_rtm import read_binary_model, smooth_velocity_model
...
Run: `python -m pytest D:\ryjin\admit_fwi\tests\test_salt_full_waveform_migration.py::test_small_demo_writes_expected_outputs -q`
...
Run: `python -m pytest D:\ryjin\admit_fwi\tests\test_salt_full_waveform_migration.py::test_small_demo_writes_chinese_report -q`
...
Run: `python -m pytest D:\ryjin\admit_fwi\tests\test_salt_full_waveform_migration.py -q`
...
Run: `python -m pytest D:\ryjin\admit_fwi\tests\test_salt_full_waveform_migration.py D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py D:\ryjin\admit_fwi\tests\test_acoustic_rtm.py D:\ryjin\admit_fwi\tests\test_optimize_existing_salt_result.py -q`

Expected: PASS。

- [ ] **Step 3: 运行盐丘小范围实验**

Run: `python D:\ryjin\admit_fwi\run_salt_full_waveform_migration.py --output-dir D:\ryjin\admit_fwi\outputs\small_salt_full_waveform_migration`

Expected: 输出 `summary.json`、`.npy` 数组、两张 PNG 图和中文 Markdown 总结。

- [ ] **Step 4: 检查结果文件**

Run: `Get-ChildItem D:\ryjin\admit_fwi\outputs\small_salt_full_waveform_migration`
...
**Files:**
- Read: `D:\ryjin\admit_fwi\outputs\small_salt_full_waveform_migration\summary.json`
- Read: `D:\ryjin\admit_fwi\outputs\small_salt_full_waveform_migration\full_waveform_migration_summary.md`
