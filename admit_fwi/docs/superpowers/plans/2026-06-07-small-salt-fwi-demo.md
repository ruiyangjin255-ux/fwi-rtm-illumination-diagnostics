...
**Goal:** 在现有 `D:\ryjin\admit_fwi` 工程中新增一个可运行的小范围 SEG/Salt FWI 演示，并输出中文解释文档和可视化结果。

**Architecture:** 新增独立脚本 `run_small_salt_fwi.py`，复用 `admit_fwi.acoustic_rtm` 中的模型读取、平滑、正演、波场和吸收边界工具。脚本内部保持函数化，便于单元测试；输出写入独立目录 `outputs/small_salt_fwi_demo`，不覆盖已有 RTM 成果。

**Tech Stack:** Python、NumPy、Matplotlib、pytest、现有 `admit_fwi.acoustic_rtm` 模块。

---

## File Structure

- Create: `D:\ryjin\admit_fwi\run_small_salt_fwi.py`
  - 负责模型裁剪、观测合成、FWI 迭代、图件输出和 `summary.json`。
- Create: `D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py`
  - 覆盖裁剪窗口、初始模型、误差计算、更新限制和一次演示运行的轻量 smoke 测试。
- Create: `D:\ryjin\admit_fwi\docs\small_salt_fwi_and_illumination.md`
...
```powershell
python -m pytest D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py -v
```
...
在 `D:\ryjin\admit_fwi\run_small_salt_fwi.py` 中创建脚本骨架，包含：
...
from admit_fwi.acoustic_rtm import smooth_velocity_model
...
**Files:**
- Modify: `D:\ryjin\admit_fwi\run_small_salt_fwi.py`
- Modify: `D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py`
...
```python
from admit_fwi.acoustic_rtm import RTMConfig
...
```powershell
python -m pytest D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py::test_compute_update_direction_returns_finite_model_shaped_array -v
```
...
```powershell
python -m pytest D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py::test_run_fwi_demo_writes_summary_and_reduces_misfit -v
```
...
创建 `D:\ryjin\admit_fwi\docs\small_salt_fwi_and_illumination.md`，内容包括：
...
**Files:**
- Read: `D:\ryjin\admit_fwi\outputs\small_salt_fwi_demo\summary.json`
...
```powershell
python D:\ryjin\admit_fwi\run_small_salt_fwi.py --iterations 3 --nt 360
```
...
```powershell
Get-ChildItem -LiteralPath D:\ryjin\admit_fwi\outputs\small_salt_fwi_demo
```
...
```powershell
rg -n "TODO|TBD|placeholder|待定|占位" D:\ryjin\admit_fwi\docs\small_salt_fwi_and_illumination.md
```
...
当前 `D:\ryjin\admit_fwi` 不是 git 仓库，跳过提交。最终答复中列出新增文件、验证命令和误差下降结果。
