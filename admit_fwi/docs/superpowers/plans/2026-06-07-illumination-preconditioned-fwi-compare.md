...
**Tech Stack:** Python、NumPy、Matplotlib、pytest、现有 `admit_fwi.acoustic_rtm` 正演与波场工具。
...
- Modify: `D:\ryjin\admit_fwi\docs\small_salt_fwi_and_illumination.md`
...
```python
from admit_fwi.run_small_salt_fwi import (
...
def test_compute_source_illumination_is_nonnegative(tmp_path):
    ...
    from admit_fwi.acoustic_rtm import forward_model
...
```powershell
python -m pytest D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py::test_compute_source_illumination_is_nonnegative D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py::test_apply_illumination_preconditioner_preserves_shape_and_finiteness -v
```
...
```python
from admit_fwi.run_small_salt_fwi import run_fwi_compare
...
```powershell
python -m pytest D:\ryjin\admit_fwi\tests\test_small_salt_fwi.py::test_run_fwi_compare_writes_compare_summary -v
```
...
python -m admit_fwi.run_small_salt_fwi --compare-illumination --iterations 3 --nt 360
```
