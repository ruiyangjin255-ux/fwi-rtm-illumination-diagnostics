...
- `admit_fwi/diagnostics/shot_partition.py`
...
- `admit_fwi/diagnostics/heldout_audit.py`
  - 提供 truth-free audit 指标：normalized L2 residual、NRMS、trace correlation、envelope error。
- `admit_fwi/diagnostics/rtm_split_consistency.py`
  - 提供 RTM split consistency 指标：image correlation、simple SSIM、local structure tensor coherence。

新增配置与脚本：

- `admit_fwi/configs/salt_reliability_gate_v1.yaml`
- `admit_fwi/scripts/replay_fwi_diagnostics.py`
- `admit_fwi/scripts/run_reliability_gate_ablation.py`
- `admit_fwi/scripts/run_holdout_gate_audit.py`
- `admit_fwi/scripts/report_reliability_gate.py`
- `admit_fwi/scripts/_common.py`

已修改正式 FWI runner：

- `admit_fwi/run_full_salt_fwi.py`
...
- `admit_fwi/tests/test_update_reliability.py`
- `admit_fwi/tests/test_matched_budget.py`
- `admit_fwi/tests/test_holdout_isolation.py`
- `admit_fwi/tests/test_reliability_outputs.py`
...
```powershell
python -m pytest -q admit_fwi\tests\test_update_reliability.py admit_fwi\tests\test_matched_budget.py admit_fwi\tests\test_holdout_isolation.py admit_fwi\tests\test_reliability_outputs.py
```
...
```powershell
python admit_fwi\scripts\replay_fwi_diagnostics.py --config admit_fwi\configs\salt_reliability_gate_v1.yaml --smoke
```
...
- `admit_fwi/outputs/salt_reliability_gate_v1/manifest.json`

已运行 hold-out split smoke：

```powershell
python admit_fwi\scripts\run_holdout_gate_audit.py --config admit_fwi\configs\salt_reliability_gate_v1.yaml --all-audit-folds --smoke
```

已写出：

- `admit_fwi/outputs/salt_reliability_gate_v1/audit/heldout_audit_manifest.json`

已生成 smoke report：

- `admit_fwi/outputs/salt_reliability_gate_v1/report/reliability_gate_smoke_report.md`

已运行极小 FWI diagnostics smoke：

```powershell
python admit_fwi\run_full_salt_fwi.py --model "admit_fwi\outputs\generated_inputs\seg676x230_from_fwi_true.bin" --output-dir admit_fwi\outputs\diagnostics_smoke --iterations 1 --nt 20 --max-shots 2 --shot-spacing 300 --optimizer steepest --no-figures --save-iteration-diagnostics --num-shot-groups 2 --diagnostics-dir admit_fwi\outputs\diagnostics_smoke_ecg
```
...
- `admit_fwi/outputs/salt_reliability_gate_v1/diagnostics/gradient_group_00.npy`
- `admit_fwi/outputs/salt_reliability_gate_v1/diagnostics/gradient_group_01.npy`
- `admit_fwi/outputs/salt_reliability_gate_v1/diagnostics/gradient_group_02.npy`
- `admit_fwi/outputs/salt_reliability_gate_v1/diagnostics/gradient_group_03.npy`
- `admit_fwi/outputs/salt_reliability_gate_v1/diagnostics/source_adjoint_energy_proxy.npy`
- `admit_fwi/outputs/salt_reliability_gate_v1/diagnostics/delta_model.npy`
- `admit_fwi/outputs/salt_reliability_gate_v1/diagnostics/step_length.json`
...
2. 开启 `--save-iteration-diagnostics`，写入 `admit_fwi/outputs/salt_reliability_gate_v1/diagnostics`。
...
```powershell
python admit_fwi\run_full_salt_fwi.py `
  --save-iteration-diagnostics `
  --num-shot-groups 4 `
  --shot-group-mode interleaved `
  --diagnostics-dir admit_fwi\outputs\salt_reliability_gate_v1\diagnostics
```

诊断量齐备后再运行：

```powershell
python admit_fwi\scripts\replay_fwi_diagnostics.py --config admit_fwi\configs\salt_reliability_gate_v1.yaml
python admit_fwi\scripts\run_reliability_gate_ablation.py --config admit_fwi\configs\salt_reliability_gate_v1.yaml
python admit_fwi\scripts\run_holdout_gate_audit.py --config admit_fwi\configs\salt_reliability_gate_v1.yaml --all-audit-folds
python admit_fwi\scripts\report_reliability_gate.py --config admit_fwi\configs\salt_reliability_gate_v1.yaml
```
