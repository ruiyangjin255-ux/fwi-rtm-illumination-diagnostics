...
> 本稿主证据来自 `D:\ryjin\rtm_acoustic` 已有 FWI/RTM 结果；`D:\ryjin\fwi_visionfm` 结果仅作为可选扩展方向，不作为本文主实验结论。
...
近年深度学习 FWI 和 OpenFWI 数据集推动了端到端速度反演研究。OpenFWI 提供了多个二维合成数据族，用于评估跨构造泛化能力。视觉基础模型和地震基础模型也开始被尝试用于地震记录到速度模型的迁移学习。但是，从本项目已有 `fwi_visionfm` 结果看，真实 DINOv2 与 LoRA/spectrogram bridge 目前仍处于 CPU smoke 或小样本阶段，只能证明技术路径可行，不能替代本文物理 FWI/RTM 的主证据。
...
`fwi_visionfm` 中已有 OpenFWI 小样本和视觉基础模型迁移实验。它们显示 LoRA + spectrogram bridge 可在目标族数值误差上出现小幅改善，但结构指标未同步改善，真实 DINOv2 仍是 smoke 级。本文可在讨论中提出一个扩展方向：用机器学习模型输出低波数速度先验或结构约束，再进入物理 FWI/RTM 验证。但在没有严格 GPU 规模、固定 split、跨族测试和 PDE 一致性约束前，不应把这些结果写成本文主创新。

## 6 可形成的创新点

### 创新点 1：文献驱动的 FWI-RTM 综合诊断框架

本文不是单独展示 FWI 或 RTM，而是把完整盐丘 FWI 残差下降、模型结构误差、更新尺度门控、RTM before/after 验证、成像条件对比、照明比例诊断和局部 FWI 机制对照放在同一复现框架中。该创新点适合作为论文主线，因为它回应了 FWI 文献中对结构指标和优化稳定性的要求，也回应了 RTM/LSRTM 文献中对成像条件和照明诊断的要求。

### 创新点 2：失败可诊断的模型质量门控

当前 FWI 图像质量不足本身不是论文失败，而是方法框架必须显式处理的问题。本文把残差下降、MAE/RMSE、edge MAE、gradient MAE、更新尺度和 RTM 图像指标联合作为门控条件，拒绝把全量 `alpha=1.0` 更新直接写成速度改进。该点比单纯展示一张较差 FWI 图更有方法价值：它说明了何时不能相信 FWI 残差下降，以及如何防止有害速度更新进入 RTM 解释。
...
### 可选创新点 6：机器学习低波数先验与物理成像闭环

若后续补充实验，可把 `fwi_visionfm` 的 OpenFWI/LoRA/spectrogram 结果作为低波数初始模型生成器，与本文 FWI/RTM 闭环结合。当前不建议写入主结果，只适合放在展望或后续工作。
...
| 图号 | 内容 | 已有/建议路径 |
|---|---|---|
| 图 1 | 文献驱动的 FWI-RTM 方法综合、证据强弱与结论边界 | `D:\ryjin\rtm_acoustic\docs\jge_main_figures\figure1_fwi_quality_gate.tiff` |
| 图 2 | RTM before/after 质量门控 FWI 速度更新验证 | `D:\ryjin\rtm_acoustic\docs\jge_main_figures\figure2_rtm_before_after_validation.tiff` |
| 图 3 | full-aperture RTM 成像条件与照明诊断 | `D:\ryjin\rtm_acoustic\docs\jge_main_figures\figure3_imaging_condition_diagnostics.tiff` |
| 图 4 | 局部 FWI 消融与保守结论边界 | `D:\ryjin\rtm_acoustic\docs\jge_main_figures\figure4_local_fwi_claim_boundary.tiff` |
| 图 5 | 盐顶、盐翼和盐下目标区照明、RTM 响应与 FWI 更新能量 | `D:\ryjin\rtm_acoustic\docs\jge_main_figures\figure5_target_zone_illumination_diagnostics.tiff` |
| 表 1 | 完整 FWI 与 RTM 参数表 | 由 `full_salt_fwi_summary.md` 和 `scheme2_report.md` 整理 |
| 表 2 | RTM before/after FWI 图像指标 | 见本文第 4.2 节 |
| 表 3 | 局部 FWI 策略对比 | 见本文第 4.5 节 |
| 表 4 | JGE 导向的创新点-程序-证据-结论边界矩阵 | `D:\ryjin\rtm_acoustic\docs\jge_revision\jge_innovation_framework.csv` |
| 表 5 | 目标区照明与 FWI-RTM 诊断指标 | `D:\ryjin\rtm_acoustic\docs\jge_revision\target_zone_illumination_metrics.csv` |
...
本文使用的代码、参数文件和主要输出位于本地项目 `D:\ryjin\rtm_acoustic`。推荐复现入口为 `python -m rtm_acoustic.run_optimized_fwi_rtm_pipeline`；若需要重新执行耗时 RTM 验证，可添加 `--run-rtm`。投稿材料打包入口为 `python -m rtm_acoustic.build_jge_submission_package`，输出目录为 `D:\ryjin\rtm_acoustic\docs\jge_submission_package_mainfigures`。完整 FWI 结果位于 `D:\ryjin\rtm_acoustic\outputs\FWI\full_salt_fwi_cg_allshots_v2`；优化 pipeline 总报告位于 `D:\ryjin\rtm_acoustic\outputs\FWI\full_salt_fwi_cg_allshots_v2\optimized_fwi_rtm_pipeline`；RTM before/after FWI 结果位于 `D:\ryjin\rtm_acoustic\outputs\RTM\before_after_fwi_alpha010_nt1200_shots12`；RTM 方案二结果位于 `D:\ryjin\rtm_acoustic\outputs\RTM\seg_salt_scheme2_full30m_nt4001_workers4`；局部 FWI 对照结果位于 `D:\ryjin\rtm_acoustic\outputs\FWI影响因素`。JGE 导向的创新点矩阵和图件 alt text 由 `python -m rtm_acoustic.build_jge_innovation_framework` 生成。已整理的公开代码仓库为 `https://github.com/ruiyangjin255-ux/fwi-rtm-illumination-diagnostics`。若投稿，需将可公开代码、参数配置、压缩后的结果表和可复现实验脚本整理到 GitHub/Zenodo，并避免上传受版权限制的原始模型文件。
