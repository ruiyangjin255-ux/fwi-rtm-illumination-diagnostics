...
from .data import ArrayBundle, assert_disjoint_splits, deterministic_subset_indices, fixed_split_indices, load_arrays
...
def build_protocol(
    ...
    source_records = np.load(source.records, mmap_mode="r")
    source_models = np.load(source.models, mmap_mode="r")
    source_n = min(len(source_records), len(source_models))
    ...
    if target is not None:
        target_records = np.load(target.records, mmap_mode="r")
        target_models = np.load(target.models, mmap_mode="r")
        target_n = min(len(target_records), len(target_models))
