...
from .plotting import plot_bridge_attributes, plot_gradient_comparison, plot_profiles, plot_training_history, plot_velocity_comparison
...
def evaluate_model(
    ...
    metrics_to_mean = ["mae", "rmse", "ssim", "edge_mae", "gradient_error"]
...
def run_single_experiment(
    ...
    metadata = {
        ...
        "metrics": {"in_family": in_summary, "cross_family": cross_summary},
    }
    with (output_dir / "metrics_summary.json").open("w", encoding="utf-8") as handle:
        json.dump(metadata, handle, indent=2, ensure_ascii=False)

    if in_cached is not None:
