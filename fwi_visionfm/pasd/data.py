...
def _sample_files(path: str | Path) -> list[Path]:
    root = Path(path)
    if not root.is_dir():
        raise ValueError(f"Expected a sample directory, got: {root}")
    files = sorted(root.glob("*.npz"))
    if not files:
        raise FileNotFoundError(f"No .npz samples found in {root}")
    return files


def _load_npz_sample(path: Path) -> tuple[np.ndarray, np.ndarray]:
    with np.load(path) as payload:
        if "records" not in payload or "velocity" not in payload:
            raise ValueError(f"{path} must contain 'records' and 'velocity' arrays.")
        records = np.asarray(payload["records"], dtype=np.float32)
        velocity = np.asarray(payload["velocity"], dtype=np.float32)
    if records.ndim != 3:
        raise ValueError(f"{path} records must be one sample [S,R,T] or [S,T,R], got {records.shape}.")
    # Local OpenFWI npz samples are [S,R,T] with receivers=70 and time=1000; PASD uses [S,T,R].
    if records.shape[1] < records.shape[2]:
        records = np.transpose(records, (0, 2, 1))
    if velocity.ndim == 3 and velocity.shape[0] == 1:
        velocity = velocity[0]
    if velocity.ndim != 2:
        raise ValueError(f"{path} velocity must be [H,W] or [1,H,W], got {velocity.shape}.")
    return records, velocity


def count_samples(records_path: str | Path, models_path: str | Path) -> int:
    records_path = Path(records_path)
    models_path = Path(models_path)
    if records_path.is_dir() and models_path.is_dir():
        return min(len(_sample_files(records_path)), len(_sample_files(models_path)))
    records_mm = np.load(records_path, mmap_mode="r")
    models_mm = np.load(models_path, mmap_mode="r")
    return min(len(records_mm), len(models_mm))


def inspect_array_source(records_path: str | Path, models_path: str | Path) -> dict[str, object]:
    records_path = Path(records_path)
    models_path = Path(models_path)
    if records_path.is_dir() and models_path.is_dir():
        files = _sample_files(records_path)
        records, velocity = _load_npz_sample(files[0])
        raw_records_shape = tuple(np.load(files[0])["records"].shape)
        return {
            "storage": "npz_directory",
            "sample_count": len(files),
            "first_sample": str(files[0]),
            "records_shape": [len(files), *records.shape],
            "velocity_shape": [len(files), *velocity.shape],
            "records_layout_detected": "[N,S,R,T]" if raw_records_shape[1] < raw_records_shape[2] else "[N,S,T,R]",
            "records_layout_canonical": "[N,S,T,R]",
            "velocity_layout_detected": "[N,H,W]",
            "dtype_records": str(records.dtype),
            "dtype_velocity": str(velocity.dtype),
        }
    records_mm = np.load(records_path, mmap_mode="r")
    models_mm = np.load(models_path, mmap_mode="r")
    return {
        "storage": "npy_arrays",
        "sample_count": min(len(records_mm), len(models_mm)),
        "records_shape": list(records_mm.shape),
        "velocity_shape": list(canonical_velocities(models_mm).shape),
        "records_layout_detected": "[N,S,T,R]",
        "records_layout_canonical": "[N,S,T,R]",
        "velocity_layout_detected": "[N,H,W]" if np.asarray(models_mm).ndim == 3 else "[N,1,H,W]",
        "dtype_records": str(records_mm.dtype),
        "dtype_velocity": str(models_mm.dtype),
    }
...
def load_arrays(
    ...
    models_path = Path(models_path)
    if records_path.is_dir() and models_path.is_dir():
        record_files = _sample_files(records_path)
        model_files = _sample_files(models_path)
        n = min(len(record_files), len(model_files))
        if max_samples is not None:
            n = min(n, int(max_samples))
        records_list: list[np.ndarray] = []
        velocity_list: list[np.ndarray] = []
        for path in record_files[:n]:
            records, velocity = _load_npz_sample(path)
            records_list.append(records)
            velocity_list.append(velocity)
        records = np.stack(records_list).astype(np.float32)
        velocities = np.stack(velocity_list).astype(np.float32)
        sample_ids = np.arange(len(record_files), dtype=np.int64)[:n] if sample_ids_path is None else load_sample_ids(sample_ids_path, len(record_files))[:n]
        ...
        return ArrayBundle(
            ...
            metadata=inspect_array_source(records_path, models_path),
        )
    ...
    source_positions = _load_geometry_array(source_positions_path, n)
    receiver_positions = _load_geometry_array(receiver_positions_path, n)
    return ArrayBundle(records=records, velocities=velocities, sample_ids=sample_ids, family=family, source_positions=source_positions, receiver_positions=receiver_positions, metadata=inspect_array_source(records_path, models_path))
